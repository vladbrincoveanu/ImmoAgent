...
- **Telegram:** optionally enter a numeric Telegram chat ID. Telegram does not
  require email confirmation for private alert delivery. A client-supplied chat
  ID is not proof of ownership for a shared channel feed.
- **Both:** configure both destinations; Telegram delivery starts immediately,
  while email delivery starts after email confirmation.
...
The shared channel feed is governed only by owner-verified alerts whose identity
matches the `COOP_CHANNEL_ALERT_OWNERS` repository secret. Set it to one or more
comma-separated email addresses and/or Telegram chat IDs. An alert must have a
confirmed email; a Telegram-only row is never owner-verified because this system
does not perform a Telegram possession check. A Telegram ID may match when it is
attached to an alert whose email has been confirmed. Unset or unconfirmed owners
fail closed, while per-user alerts remain unaffected.
...
- `COOP_CHANNEL_ALERT_OWNERS` — email address(es) and/or Telegram chat ID(s) on
  email-confirmed alerts whose filters define the shared co-op channel feed.
